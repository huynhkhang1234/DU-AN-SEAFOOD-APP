...
DECLARE @TongThanhTien FLOAT;
	...
	SELECT top 1  @TongThanhTien as tong ,TienNhan FROM DanhSachDonHang dsdh join 
	DonHang dh 
...
--DECLARE @TongTien money, @SoLuong int;
	...
	join NhanSu ns on ns.MaNhanVien = hdct.MaNhanVien
	GROUP BY hd.MaHoaDon, ns.HoVaTen  --, sum(TongThanhTien),  sum(SoLuong), 
	

	SELECT hd.MaHoaDon, sum(TongThanhTien) as N'TongThanhTien' ,  sum(SoLuong) as 'SoLuong', hdct.MaNhanVien 
	FROM HoaDon hd join HoaDonChiTiet hdct on hd.MaHoaDon = hdct.MaHoaDon
	GROUP BY hd.MaHoaDon, hdct.MaNhanVien
